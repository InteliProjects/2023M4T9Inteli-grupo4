...
void publishAlertStatus(bool data){
  char topic[100];
  sprintf(topic, "/v1.6/devices/%s", DEVICE_LABEL);
  char payload[100];
  sprintf(payload, "{\"alertstatus\": %d}", data);
  client.publish(topic, payload);
}
...
void sendDataToFile(float weight, float force, float temperature){
  logFile = SD.open("/log.csv", FILE_APPEND); // Abre o arquivo em modo de anexação
  if (logFile) {
   ...
   // if(millis() - lastTimeClosed >= 10000){
  //   logFile.close();
  //   Serial.println("Arquivo fechado");
  //   logFile = SD.open("/log.csv", FILE_APPEND);
  //   Serial.println("Arquivo reaberto");
  //   if (!logFile) {
  //     Serial.println("Erro ao abrir log.csv");
  //   }

  //   lastTimeClosed = millis();
  // }
}
...
void createFile(){
  ...
  if (logFile) {
    logFile.println("Peso (kg),Força (N),Temperatura (°C)");
...
void loop() {
  ...
  // Envia dos dados a cada 1 segundo
  // if(millis() - lastTimeDataSend >= 1000){
    ...
    publishAlertStatus(wms.alertStatus);
  ...
  sendDataToFile(wms.weightBalance, wms.newtonBalance, temperature);
