...
void publishForce(){
  float getWeight = wms.getWeight();
  float getForce = wms.convertWeightToNewton(getWeight);
  char topic[100];
  sprintf(topic, "/v1.6/devices/%s", DEVICE_LABEL);
  char payload[100];
  sprintf(payload, "{\"force\": %f}", getForce);
  client.publish(topic, payload);
}

void publishUsedSpaceLimit(){
  float valueLimit =  wms.alertLimit;
  float getWeight = wms.getWeight();
  float getForce = wms.convertWeightToNewton(getWeight);
  float usedSpaceLimit = ((getForce*100)/valueLimit);
  if (valueLimit == 0 && usedSpaceLimit <= 0){
    usedSpaceLimit = 0;
  }
  char topic[100];
  sprintf(topic, "/v1.6/devices/%s", DEVICE_LABEL);
  ...
  client.publish(topic, payload);
  Serial.println(String(usedSpaceLimit));
}

void publishAlertStatus(){
  bool getAlertStatus = wms.alertStatus;
  char topic[100];
  sprintf(topic, "/v1.6/devices/%s", DEVICE_LABEL);
  char payload[100];
  sprintf(payload, "{\"alertstatus\": %d}", getAlertStatus);
  client.publish(topic, payload);
}
